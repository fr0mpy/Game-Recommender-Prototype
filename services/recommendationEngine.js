...
const EXPLANATION_PROMPT_FILE = path.join(
  __dirname,
  "..",
  "prompts",
  "recommendation-explanation.md"
);
const SIMILARITY_PROMPT_FILE = path.join(
  __dirname,
  "..",
  "prompts",
  "similarity-engine.md"
);
...
async function calculateBatchLLMSimilarity(
  targetGame,
  candidateGames,
  playerContext = null,
  userWeights = null
  ...
  if (!anthropic) {
    ...
    return candidateGames.map((game) => ({
      gameId: game.id,
      similarity: calculateAlgorithmicSimilarity(
        targetGame,
        game,
...
PLAYER CONTEXT: ${JSON.stringify(
    ...
    if (userWeights) {
      const weightPercentages = {
        ...
        hitFrequency: Math.round(userWeights.hitFrequency * 100),
        artStyle: Math.round(userWeights.artStyle * 100),
        audioVibe: Math.round(userWeights.audioVibe * 100),
        visualDensity: Math.round(userWeights.visualDensity * 100),
        reelLayout: Math.round(userWeights.reelLayout * 100),
      };

      // Generate exclusive mode text - detect any 100% weight
      const exclusiveWeight = Object.entries(weightPercentages).find(([key, value]) => value === 100);
      const exclusiveMode = exclusiveWeight
        ? `🎯 ${exclusiveWeight[0].toUpperCase()} EXCLUSIVE MODE: Score games by ${exclusiveWeight[0]} similarity ONLY. Ignore all other factors completely.`
        : "🎯 WEIGHTED MODE: Apply user percentages proportionally.";

      batchPrompt += `

🎚️ USER-DEFINED SIMILARITY WEIGHTS:

Apply these exact weightings to similarity calculations:
{
  ...
  "hitFrequency": ${weightPercentages.hitFrequency}%,
  "artStyle": ${weightPercentages.artStyle}%,
  "audioVibe": ${weightPercentages.audioVibe}%,
  "visualDensity": ${weightPercentages.visualDensity}%,
  "reelLayout": ${weightPercentages.reelLayout}%
}

${exclusiveMode}

Weight each similarity factor by the percentage above. 0% means ignore completely, 100% means focus exclusively on that factor.`;

      console.log(
        `🎚️ Using dynamic weights for ${batchGames.length} games: Theme=${weightPercentages.theme}%, Volatility=${weightPercentages.volatility}%, Studio=${weightPercentages.studio}%, BonusFreq=${weightPercentages.bonusFrequency}%`
      );

      if (exclusiveWeight) {
        console.log(
          `🎯 ${exclusiveWeight[0].toUpperCase()} FOCUS MODE: Analyzing games purely on ${exclusiveWeight[0]} similarity`
        );
      }
    }
    ...
    console.log(
      `🤖 Batch LLM Similarity: ${targetGame.title} vs ${batchGames.length} games completed`
    );
    console.log(
      `🔍 Sample results: ${results
        .slice(0, 3)
        .map((r) => `${r.game_id}=${r.similarity_score}%`)
        .join(", ")}`
    );
    ...
    return batchGames.map((game) => {
      const similarity = calculateAlgorithmicSimilarity(
        targetGame,
        game,
        userWeights || DEFAULT_WEIGHTS
      );
...
async function calculateLLMSimilarity(
  game1,
  game2,
  playerContext = null,
  userWeights = null
) {
  const batchResults = await calculateBatchLLMSimilarity(
    game1,
    [game2],
    playerContext,
    userWeights
...
function calculateAlgorithmicSimilarity(
  game1,
  game2,
  ...
  const is100PercentMode = Object.values(weights).some(
  ...
  if (
    game1.theme &&
    game2.theme &&
    Array.isArray(game1.theme) &&
    Array.isArray(game2.theme)
  ) {
    if (weights.theme >= 0.999) {
      ...
      debugInfo.push(
        `theme: ${themeScore.toFixed(3)} (100% mode - ${intersection.length}/${
          union.size
        } Jaccard)`
      );
    } else {
      ...
      if (game1.theme.length > 0) {
        ...
        debugInfo.push(
          `theme: ${themeScore.toFixed(3)} (${themeOverlap}/${
            game1.theme.length
          } overlap)`
        );
      }
    ...
    debugInfo.push(
      `theme: 0 (missing data - g1:${!!game1.theme}, g2:${!!game2.theme})`
    );
  }
  score += themeScore;
  ...
  if (weights.volatility >= 0.999) {
    ...
    if (vol1 === vol2) {
      ...
      debugInfo.push(
        `volatility: ${weights.volatility.toFixed(3)} (exact match ${
          game1.volatility
        })`
      );
    } else if (Math.abs(vol1 - vol2) === 1) {
      ...
      debugInfo.push(
        `volatility: ${volatilityScore.toFixed(3)} (close match ${
          game1.volatility
        }/${game2.volatility})`
      );
    } else {
      debugInfo.push(
        `volatility: 0 (no match ${game1.volatility}/${game2.volatility})`
      );
    }
  }
  ...
  if (weights.studio >= 0.999) {
    ...
    if (game1.studio && game2.studio && game1.studio === game2.studio) {
      ...
      debugInfo.push(
        `studio: ${weights.studio.toFixed(3)} (match: ${game1.studio})`
      );
    } else {
      debugInfo.push(
        `studio: 0 (${game1.studio || "null"} vs ${game2.studio || "null"})`
      );
    }
  }
  score += studioScore;
  ...
  if (
    game1.mechanics &&
    game2.mechanics &&
    Array.isArray(game1.mechanics) &&
    Array.isArray(game2.mechanics)
  ) {
    if (weights.mechanics >= 0.999) {
      ...
      debugInfo.push(
        `mechanics: ${mechanicsScore.toFixed(3)} (100% mode - ${
          intersection.length
        }/${union.size} Jaccard)`
      );
    } else {
      // Regular weighted mechanics scoring
      const mechanicsOverlap = game1.mechanics.filter((m) =>
        game2.mechanics.includes(m)
      ).length;
      if (game1.mechanics.length > 0) {
        mechanicsScore =
          (mechanicsOverlap / game1.mechanics.length) * weights.mechanics;
        debugInfo.push(
          `mechanics: ${mechanicsScore.toFixed(3)} (${mechanicsOverlap}/${
            game1.mechanics.length
          } overlap)`
        );
      }
    }
    ...
    debugInfo.push(
      `mechanics: 0 (missing data - g1:${!!game1.mechanics}, g2:${!!game2.mechanics})`
    );
  }
  ...
  if (game1.rtp && game2.rtp && rtpWeight > 0) {
    ...
    if (rtpDiff < 0.5) {
      ...
      debugInfo.push(
        `rtp: ${rtpWeight.toFixed(3)} (exact match ${game1.rtp}%)`
      );
    } else if (rtpDiff < 2) {
      rtpScore = rtpWeight * 0.5; // Within 2% = half match
      debugInfo.push(
        `rtp: ${(rtpWeight * 0.5).toFixed(3)} (close ${game1.rtp}% vs ${
          game2.rtp
        }%)`
      );
    } else {
  ...
  if (game1.maxWin && game2.maxWin && maxWinWeight > 0) {
    const ratio =
      Math.min(game1.maxWin, game2.maxWin) /
      Math.max(game1.maxWin, game2.maxWin);
    if (ratio > 0.8) {
      maxWinScore = maxWinWeight; // Within 20% = full match
      debugInfo.push(
        `maxWin: ${maxWinWeight.toFixed(3)} (similar ${game1.maxWin}x vs ${
          game2.maxWin
        }x)`
      );
    } else if (ratio > 0.5) {
      ...
      debugInfo.push(
        `maxWin: ${(maxWinWeight * 0.5).toFixed(3)} (moderate ${
          game1.maxWin
        }x vs ${game2.maxWin}x)`
      );
    } else {
  ...
  if (
    game1.features &&
    game2.features &&
    Array.isArray(game1.features) &&
    Array.isArray(game2.features) &&
    featuresWeight > 0
  ) {
    // Check for critical features
    const hasProgressive1 = game1.features.some((f) =>
      f.toLowerCase().includes("progressive")
    );
    const hasProgressive2 = game2.features.some((f) =>
      f.toLowerCase().includes("progressive")
    );
    ...
    if (hasProgressive1 === hasProgressive2) {
      ...
      if (union.size > 0) {
        ...
        debugInfo.push(
          `features: ${featuresScore.toFixed(3)} (${intersection.length}/${
            union.size
          } overlap, prog:${hasProgressive1})`
        );
      }
    } else {
  ...
  if (game1.pace && game2.pace && paceWeight > 0) {
    if (game1.pace === game2.pace) {
      ...
      debugInfo.push(
        `pace: ${paceWeight.toFixed(3)} (exact match ${game1.pace})`
      );
    } else if (
      ...
      debugInfo.push(
        `pace: ${(paceWeight * 0.5).toFixed(3)} (adjacent ${game1.pace}/${
          game2.pace
        })`
      );
    } else {
  ...
  if (game1.hitFrequency && game2.hitFrequency && hitFreqWeight > 0) {
    ...
    if (freqDiff < 5) {
      ...
      debugInfo.push(
        `hitFreq: ${hitFreqWeight.toFixed(
          3
        )} (close ${game1.hitFrequency.toFixed(1)}%)`
      );
    } else if (freqDiff < 15) {
      hitFreqScore = hitFreqWeight * 0.5; // Within 15% = half match
      debugInfo.push(
        `hitFreq: ${(hitFreqWeight * 0.5).toFixed(
          3
        )} (moderate ${game1.hitFrequency.toFixed(
          1
        )}% vs ${game2.hitFrequency.toFixed(1)}%)`
      );
    } else {
      debugInfo.push(
        `hitFreq: 0 (${game1.hitFrequency.toFixed(
          1
        )}% vs ${game2.hitFrequency.toFixed(1)}%)`
      );
    }
  ...
  if (game1.bonusFrequency && game2.bonusFrequency && bonusFreqWeight > 0) {
    ...
    if (bonusDiff < 2) {
      ...
      debugInfo.push(
        `bonusFreq: ${bonusFreqWeight.toFixed(
          3
        )} (similar ${game1.bonusFrequency.toFixed(1)}%)`
      );
    } else if (bonusDiff < 5) {
      bonusFreqScore = bonusFreqWeight * 0.5; // Within 5% = half match
      debugInfo.push(
        `bonusFreq: ${(bonusFreqWeight * 0.5).toFixed(
          3
        )} (moderate ${game1.bonusFrequency.toFixed(
          1
        )}% vs ${game2.bonusFrequency.toFixed(1)}%)`
      );
    } else {
      debugInfo.push(
        `bonusFreq: 0 (${game1.bonusFrequency.toFixed(
          1
        )}% vs ${game2.bonusFrequency.toFixed(1)}%)`
      );
    }
  ...
  if (game1.artStyle && game2.artStyle && artStyleWeight > 0) {
    if (game1.artStyle === game2.artStyle) {
      ...
      debugInfo.push(
        `artStyle: ${artStyleWeight.toFixed(3)} (match ${game1.artStyle})`
      );
    } else {
  ...
  if (game1.audioVibe && game2.audioVibe && audioWeight > 0) {
    if (game1.audioVibe === game2.audioVibe) {
      ...
      debugInfo.push(
        `audio: ${audioWeight.toFixed(3)} (match ${game1.audioVibe})`
      );
    } else {
  ...
  if (game1.visualDensity && game2.visualDensity && densityWeight > 0) {
    if (game1.visualDensity === game2.visualDensity) {
      ...
      debugInfo.push(
        `density: ${densityWeight.toFixed(3)} (match ${game1.visualDensity})`
      );
    } else {
      debugInfo.push(
        `density: 0 (${game1.visualDensity} vs ${game2.visualDensity})`
      );
    }
  }
  score += densityScore;
  ...
  if (game1.reelLayout && game2.reelLayout && reelWeight > 0) {
    if (game1.reelLayout === game2.reelLayout) {
      ...
      debugInfo.push(
        `reel: ${reelWeight.toFixed(3)} (match ${game1.reelLayout})`
      );
    } else {
  ...
  if (global.debugCalculationCount < 3) {
    ...
    console.log(
      `🔍 Final score: ${finalScore.toFixed(4)} (${Math.round(
        finalScore * 100
      )}%)`
    );
    console.log(
      `🔍 Weight validation - Sum: ${Object.values(weights)
        .reduce((a, b) => a + b, 0)
        .toFixed(4)}`
    );
...
async function getRecommendations(
  ...
  const cacheKey = `${gameId}-${JSON.stringify(
    contextAdjustedWeights
  )}-${JSON.stringify(
    playerContext?.focusLevel || "none"
  ...
  if (playerContext) {
    eligibleGames = filterGamesByContext(
      eligibleGames,
      playerContext,
      targetGame
    );
  ...
  console.log(
    `🎚️  WEIGHT VALIDATION: Total = ${totalWeight.toFixed(
      4
    )} (should be ~1.0000)`
  );
  ...
  if (Math.abs(totalWeight - 1.0) > 0.01) {
    console.log(
      `⚠️  WARNING: Weight sum ${totalWeight.toFixed(
        4
      )} is not close to 1.0 - this may cause scoring issues!`
    );
  }
  ...
  if (recommendationEngine === "llm") {
    ...
    console.log(
      "   🧠 Using AI for semantic understanding of game similarities"
    );
    ...
    console.log(
      `   🎯 Target: "${targetGame.title}" (${targetGame.theme?.join(
        "/"
      )} themes)`
    );
    ...
    console.log(
      `   ✅ LLM batch completed in ${batchEndTime - batchStartTime}ms`
    );
    console.log(
      `   📊 Results received: ${batchResults.length} game similarities`
    );
    ...
    for (const result of batchResults) {
      ...
      if (game) {
        ...
        const finalConfidence = Math.round(
          Math.min((result.similarity + contextBonus) * 100, 100)
        );
        ...
        if (finalScore > 0.7 || processedCount <= 5) {
          console.log(
            `   ${processedCount}. "${game.title}" - ${Math.round(
              finalScore * 100
            )}% similarity ${
              contextBonus > 0
                ? `(+${Math.round(contextBonus * 100)}% context bonus)`
                : ""
            }`
          );
        }
    ...
    for (const game of eligibleGames) {
      ...
      const finalConfidence = Math.round(
        Math.min((similarity + contextBonus) * 100, 100)
      );
      ...
      if (processedCount < 3) {
        ...
        console.log(
          `   🎨 Themes: [${game.theme?.join(", ") || "N/A"}] vs [${
            targetGame.theme?.join(", ") || "N/A"
          }]`
        );
        console.log(
          `   ⚡ Volatility: "${game.volatility}" vs "${targetGame.volatility}"`
        );
        ...
        console.log(
          `   ⚠️  Expected vs Actual: Expected=${Math.round(
            similarity * 100
        ...
        if (finalConfidence === 100 && similarity < 0.95) {
          console.log(
            `   🚨 POTENTIAL BUG: Low similarity (${similarity.toFixed(
              4
            )}) showing as 100%!`
          );
        }
    ...
    console.log(
      `   ✅ Algorithmic processing completed in ${
        algoEndTime - algoStartTime
      }ms`
    );
    console.log(
      `   📊 Processed ${processedCount} games with mathematical similarity`
    );
  }
  ...
  console.log(
    `   📊 Total recommendations generated: ${recommendations.length}`
  );
  ...
  sortedRecommendations.forEach((rec, i) => {
    ...
    console.log(
      `   ${i + 1}. "${rec.game.title}" - ${similarity}% match ${
        hasAnalysis ? "(with LLM analysis)" : "(algorithmic)"
      }`
    );
    if (rec.game.theme) {
...
function filterGamesByContext(games, context, targetGame) {
  return games.filter((game) => {
    ...
    if (
      (context.atWork || context.focusLevel === "split-attention") &&
    ...
    if (context.deviceType === "mobile" && game.mobileOptimized === false) {
...
function calculateContextBonus(game, context) {
  ...
  if (context.timeOfDay === "morning" && game.theme) {
    if (
      game.theme.some((t) =>
        ["Coffee", "Breakfast", "Fresh", "Energy"].includes(t)
      )
    ) {
      bonus += 0.03;
    }
  ...
  if (context.timeOfDay === "late-night" && game.theme) {
    if (
      game.theme.some((t) =>
        ["Dreams", "Stars", "Moon", "Mystical", "Zen"].includes(t)
      )
    ) {
      bonus += 0.03;
    }
  ...
  if (context.isWeekend && game.theme) {
    if (
      game.theme.some((t) =>
        ["Adventure", "Quest", "Exploration", "Journey"].includes(t)
      )
    ) {
      bonus += 0.02;
    }
  ...
  if (context.activeSportsEvents && game.theme) {
    if (
      game.theme.some((t) =>
        ["Sports", "Football", "Champions", "Tournament"].includes(t)
      )
    ) {
      bonus += 0.04;
    }
...
async function generateMatchExplanation(
  ...
  try {
    ...
    const userPrompt = `Selected Game: ${selectedGame.title} by ${
      selectedGame.studio
    }
...
Explain why "${recommendedGame.title}" is a good match for someone who likes "${
      selectedGame.title
    }".`;
    ...
    return (
      ...
      generateBasicExplanation(selectedGame, recommendedGame, confidence)
    );
  } catch (error) {
...
function generateBasicExplanation(selectedGame, recommendedGame, confidence) {
  ...
  if (selectedGame.theme && recommendedGame.theme) {
    const sharedThemes = selectedGame.theme.filter((t) =>
      recommendedGame.theme.includes(t)
    );
  ...
  if (selectedGame.mechanics && recommendedGame.mechanics) {
    const sharedMechanics = selectedGame.mechanics.filter((m) =>
      recommendedGame.mechanics.includes(m)
    );
    if (sharedMechanics.length > 0) {
      similarities.push(
        `similar mechanics (${sharedMechanics.slice(0, 2).join(", ")})`
      );
    }
  }
  ...
  return `Strong ${confidence}% match due to ${similarities
    .slice(0, 2)
    .join(" and ")}.`;
}
